...
/** Transfer dialog — moves funds between two accounts. */
export function TransferDialog({ open, onOpenChange, defaultFromId }: Props) {
  const { transferBetweenAccounts } = useTransfers()
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-full max-w-[440px] sm:max-w-[440px] ring-0 shadow-none bg-mech-paper border border-mech-ink-20 rounded-none">
        <DialogHeader>
          <DialogTitle className="font-grotesk font-semibold text-base text-mech-dark">Transfer Funds</DialogTitle>
        </DialogHeader>

        <form onSubmit={form.handleSubmit(handleSubmit)} className="flex flex-col gap-4 pt-2">
          <div className="flex flex-col gap-1.5">
            <FieldTooltip label="From Account" required tooltip="Account to transfer out of." />
            <select
          ...
          <div className="flex flex-col gap-1.5">
            <FieldTooltip label="To Account" required tooltip="Account to receive the funds." />
            <select
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="flex flex-col gap-1.5">
              <FieldTooltip label="Amount" required tooltip="Amount to transfer." />
            ...
            <div className="flex flex-col gap-1.5">
              <FieldTooltip label="Date" required tooltip="Date of transfer." />
              <input
                type="date"
                {...form.register('date')}
          ...
          <div className="flex gap-2 pt-1">
            ...
            >
              {form.formState.isSubmitting ? 'Saving...' : 'Transfer'}
            </button>
          </div>
        </form>
      </DialogContent>
